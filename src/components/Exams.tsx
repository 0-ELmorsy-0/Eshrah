    ...
    <div className="space-y-6">
      ...
      <div className="bg-bg-card border border-white/5 rounded-3xl overflow-hidden">
        ...
        <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 bg-white/[0.02]">
          <h3 className="text-xl font-bold text-white">نتائج الامتحانات</h3>
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.01]">
                ...
                <th className="py-5 px-6 text-sm font-semibold text-slate-300">الامتحان</th>
                <th className="py-5 px-6 text-sm font-semibold text-slate-300">الدرجة</th>
                <th className="py-5 px-6 text-sm font-semibold text-slate-300 text-center">المدة</th>
                <th className="py-5 px-6 text-sm font-semibold text-slate-300">التاريخ</th>
            ...
            <tbody className="divide-y divide-white/5">
              {examsData.map((exam) => (
                <tr key={exam.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-white font-medium whitespace-nowrap">{exam.name}</span>
                      </div>
                    </div>
                  </td>
                  <td className="py-4 px-6 text-slate-300 text-sm max-w-[150px]">{exam.course}</td>
                  <td className="py-4 px-6 text-slate-300 text-sm max-w-[200px]">{exam.exam}</td>
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3 w-32">
                      <span className="text-white font-medium w-12">{exam.score}/100</span>
                  ...
                  <td className="py-4 px-6 text-slate-400 text-sm text-center">
                    {exam.duration.split(" ")[0]}
                    <br />
                    <span className="text-xs">{exam.duration.split(" ")[1]}</span>
                  </td>
                  <td className="py-4 px-6 text-slate-400 text-sm font-mono tracking-tight">{exam.date}</td>
                  <td className="py-4 px-6">
                    <span className={cn("px-3 py-1 text-xs font-medium border rounded-full whitespace-nowrap", exam.statusColor)}>
                      {exam.status}
                    </span>
                  </td>
                </tr>
