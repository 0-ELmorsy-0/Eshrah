  ...
  return (
    <header className="flex items-center justify-between mt-2 mb-8">
      <div className="flex items-center gap-4">
        <button className="md:hidden p-2 bg-bg-card rounded-xl text-slate-300">
          <Menu className="w-5 h-5" />
        </button>
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">{activeTab}</h2>
          <p className="text-sm text-slate-400">مرحباً بك في نظام إدارة إشرحــ طب</p>
        </div>
      </div>

      <div className="flex items-center gap-6">
        <button className="relative p-2 text-slate-300 hover:text-white transition-colors bg-bg-card rounded-full">
